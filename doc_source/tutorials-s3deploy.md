...
**Note**  
This is not the source bucket for your pipeline's source code\. This is the artifact store for your pipeline\. A separate artifact store, such as an Amazon S3 bucket, is required for each pipeline\. The artifact store you create or choose must be in the same region as your pipeline\. Later, if you add cross\-region actions, you need another artifact store in each region where you have actions\. For more information, see [A Quick Look at Input and Output Artifacts](welcome.md#welcome-introducing-artifacts)\.
