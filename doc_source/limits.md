...
|  |  | 
| --- |--- |
|  AWS Regions where you can create a pipeline  |  US East \(Ohio\) US East \(N\. Virginia\) US West \(N\. California\) US West \(Oregon\) Canada \(Central\) Europe \(Stockholm\) Europe \(Ireland\) Europe \(London\) Europe \(Paris\) Europe \(Frankfurt\) Asia Pacific \(Hong Kong\)\* Asia Pacific \(Mumbai\) Asia Pacific \(Tokyo\) Asia Pacific \(Seoul\) Asia Pacific \(Singapore\) Asia Pacific \(Sydney\) South America \(São Paulo\) AWS GovCloud \(US\-West\)  | 
