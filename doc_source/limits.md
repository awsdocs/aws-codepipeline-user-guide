...
|  Maximum number of pipelines set to polling for source changes, per AWS Region  |  300  This quota is fixed and cannot be changed\. If you reach the limit for polling pipelines, you can still configure additional pipelines that use event\-based change detection\. For more information, see [ Source actions and change detection methods](pipelines-about-starting.md#change-detection-methods)\.1   | 
...
| Length of time before a manual approval action times out |  7 days  | 
